...
tokens_to_verify_identificador = {"TIPO_DATO_MATRIZ","TIPO_DATO_VECTOR","RETORNO","TIPO_DATO", "FUNCION", "CUANDO", "MIENTRAS", "DE","OPERADOR_COMPARACION","RANGO","OPERADOR_LOGICO_AND","OPERADOR_LOGICO_OR","OPERADOR_ARITMETICO","INICIO","ASIGNACION","FIN_DE_INSTRUCCION", "PARENTESIS_IZQ", "IDENTIFICADOR","FIN"}
...
def syntactic_analysis(tokens):
    ...
    for token in tokens:
        if token[0] == 'TIPO_DATO':
            ...
            if top(pila_data_type)[0] in tokens_num_entero_flotante or top(pila_data_type)[0] in tokens_cadena_caracter or top(pila_data_type)[0] == 'PARENTESIS_DER' or top(pila_data_type)[0] == "LEER":
                pila_data_type.append(token)
            else:
                return f"Error sintactico en {token[1]}"
