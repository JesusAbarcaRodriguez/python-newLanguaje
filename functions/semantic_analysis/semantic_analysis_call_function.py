def semantic_analysis_call_function(principal,function_name, parameters_input):
    iterator = 0
    keys_functions = principal.functions.keys()
    if function_name in keys_functions:
        function_obj = principal.functions[function_name]
        expected_parameters = function_obj.parameters
        if len(parameters_input) == len(expected_parameters):
            for param_name, param_data  in expected_parameters.items():
                if param_data[0] == parameters_input[iterator][0]:
                    principal.functions[function_name].parameters[param_name][1] = parameters_input[iterator][1]
                    iterator += 1
        ...
        return f"Error: La función {function_name} no ha sido definida"
    return 'OK'