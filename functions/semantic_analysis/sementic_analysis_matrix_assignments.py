...
from functions.utils.utils import error_message, is_array_call, is_called_fuction_procedure, is_declarate_matrix, is_declared_variable, is_matrix_call, is_read, is_same_type
    ...
    if is_declarate_matrix(tokens,i,matrix):
        ...
        while tokens[i][0] != 'FIN_DE_INSTRUCCION':
            if is_called_fuction_procedure(tokens,i):
                ...
                if not (matrix_to_assign[0] == 'ENTERO' or matrix_to_assign[0] == 'FLOTANTE'):
                    return f"Error la asignación es de tipos diferentes, la variable '{variable_name}' no es de tipo {tokens[i][0]}"
                i += 1
            elif tokens[i][0] == 'NUMERO_FLOTANTE':
                if not (matrix_to_assign[0] == 'FLOTANTE' ):
                    return f"Error la asignación es de tipos diferentes, la variable '{variable_name}' no es de tipo {tokens[i][0]}"
                i += 1
            elif tokens[i][0] == 'VALOR_BOOLEANO':
                if not (matrix_to_assign[0] == 'BOOLEANO'):
                    return f"Error la asignación es de tipos diferentes, la variable '{variable_name}' no es de tipo {tokens[i][0]}"
                i += 1
            elif tokens[i][0] == "OPERADOR_ARITMETICO":
                i += 1
            elif tokens[i][0] == "VALOR_CARACTER":
                if not matrix_to_assign[0] == 'CARACTER':
                    return f"Error la asignación es de tipos diferentes, la variable '{variable_name}' no es de tipo {tokens[i][0]}"
                i +=1
            elif tokens[i][0] == "CADENA_LITERAL":
                if not matrix_to_assign[0] == 'CADENA':
                    return f"Error la asignación es de tipos diferentes, la variable '{variable_name}' no es de tipo {tokens[i][0]}"
                i +=1
            else:
